...
path = "../csv/result2.csv"
with open(path, "r", newline="") as f:
    lines = f.readlines()

if lines and lines[0].startswith("api,threads,time,benchmarkNumber") and "serial," in lines[0] and "\n" not in lines[0]:
    header = "api,threads,time,benchmarkNumber\n"
    glued = lines[0]
    start = glued.find("serial,")
    lines = [header, glued[start:]]
...
serial_base = None
for th, tm in zip(x3, time_serial):
    if th == 1:
        serial_base = tm
        break
if serial_base is None:
    raise ValueError("1 thread-ийн үр дүн олдсонгүй.")

def to_speedup_with_base(base, times):
    return [base / tm for tm in times]

speedup_openmp = to_speedup_with_base(serial_base, time_openmp)
speedup_threads = to_speedup_with_base(serial_base, time_threads)
